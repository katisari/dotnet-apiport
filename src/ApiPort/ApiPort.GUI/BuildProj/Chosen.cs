        ...
        public static void Configure(string path, string config, string plat, string json1Path)
        {
            ...
            if (File.Exists(path: project.GetProperty("TargetPath").EvaluatedValue.ToString()))
            {
                ...
                using (JsonWriter writer = new JsonTextWriter(sw))
                {
                    ...
                    Info info = new Info(File.Exists(path: project.GetProperty("TargetPath").EvaluatedValue.ToString()), null, null, targetPathString, assemblyCode, assembly.Location, false);
            ...
            else
            {
                ...
                using (JsonWriter writer = new JsonTextWriter(sw))
                {
                    Info info = new Info(File.Exists(path: project.GetProperty("TargetPath").EvaluatedValue.ToString()), null, null, null, null, null, false);
                    serializer.Serialize(writer, info);
                    sw.Close();
                    writer.Close();
                }
